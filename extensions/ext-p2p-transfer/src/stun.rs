...
impl StunClient {
    ...
    fn detect_with_server(
        ...
        local_socket.set_read_timeout(Some(Duration::from_millis(self.timeout_ms))).ok()?;
...
fn build_binding_request(change_ip: bool, change_port: bool) -> Vec<u8> {
    ...
    if change_ip { change_bits |= 0x04; }
    if change_port { change_bits |= 0x02; }
