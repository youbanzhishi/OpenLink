    ...
    async fn authenticate(&self, credentials: &Credentials) -> AuthResult {
        ...
        match self.keys.get(key_str) {
            Some(entry) if entry.active => {
                AuthResult::success(&entry.name, entry.scopes.clone())
            }
...
mod tests {
    ...
    async fn test_api_key_auth() {
        ...
        auth.add_key("secret-key-123", "test-user", vec!["read".to_string(), "write".to_string()]);
    ...
    async fn test_auth_middleware_extract_bearer() {
        let creds = AuthMiddleware::extract_credentials(
