...
fn default_partition_threshold() -> f64 { 0.3 }
fn default_cache_ttl() -> u64 { 300 }
fn default_degradation() -> DegradationStrategy { DegradationStrategy::CloudRelay }
...
impl DecentralizedCapabilityRouter {
    ...
    pub async fn handle_capability_gossip(&self, gossip: &CapabilityGossip) -> usize {
        ...
        for announcement in &gossip.capabilities {
            let route = routes.entry(announcement.capability_id.clone()).or_insert_with(|| CapabilityRoute {
...
mod tests {
    ...
    async fn test_register_and_find_capability() {
        ...
        router.register_capability(provider, "text-gen", "Text Generation").await;
    ...
    async fn test_capability_gossip() {
        ...
        let gossip = router.create_capability_gossip(vec![
