...
use openlink_core::extension_search::{ExtensionSearchRequest, ExtensionSearchResponse, ExtensionSchema, ExtensionType};
...
pub async fn execute_extension(
    ...
    let ext_type = ExtensionType::try_from_str(&ext_type_str)
    ...
    match ext_type {
        ExtensionType::Action => {
            ...
            let handler: Arc<dyn openlink_core::ActionHandler> = {
                ...
                inner.get_action_handler(&name)
                    .ok_or_else(|| (StatusCode::NOT_FOUND, format!("Action '{}' not found", name)))?
            };
            let target = Target { action: Action::Custom(name), params: arguments };
            let result = handler.execute(&ctx, &target).await
                .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("{}", e)))?;
            Ok(Json(ExecuteResponse { ok: true, result: Some(serde_json::to_value(result).unwrap_or_default()), error: None }))
        }
        ExtensionType::Condition => {
            let handler: Arc<dyn openlink_core::ConditionHandler> = {
                let registry = state.search_registry.read().await;
                let inner = registry.inner();
                inner.get_condition_handler(&name)
                    .ok_or_else(|| (StatusCode::NOT_FOUND, format!("Condition '{}' not found", name)))?
            };
            let matched = handler.evaluate(&ctx, &arguments).await
                .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("{}", e)))?;
            Ok(Json(ExecuteResponse { ok: true, result: Some(serde_json::json!({ "matched": matched })), error: None }))
        }
        _ => Err((StatusCode::BAD_REQUEST, format!("Cannot execute {:?} directly", ext_type))),
    }
}
